// Typical supermarket unit prices (USD), ported from Huddle.
// Country multipliers convert to local currency. Optional AI overrides refresh
// a small catalog monthly; every recipe is then priced locally from ingredients.

import { roundMoney } from '../stash/money';
import { parseAmount } from './amounts';
import type { GroceryMarket } from './markets';
import type { Ingredient, Recipe, RecipeCostEstimate, RecipeCostLine } from './types';
...
export function localFromUsd(usd: number, countryCode?: string): number {
  return roundMoney(usd * getCurrencyConfig(countryCode).multiplier);
}
...
export type PriceOverrideEntry = { priceUSD: number; baseAmount: number; baseUnit: string };
export type PriceOverrideMap = Record<string, PriceOverrideEntry>;

export type IngredientPriceBook = {
...
export type RecipeCost = {
  ...
  lines: { ingredient: Ingredient; usd: number }[];
};

function scaledOverrideCost(name: string, amount: string | undefined, match: { entry: CatalogIngredient; priceUSD: number }): number {
  const parsed = parseAmount(amount);
  const converted = parsed ? toBaseUnit(parsed.value, parsed.unit) : null;
  if (converted && converted.unit === match.entry.baseUnit) {
    return (converted.value / match.entry.baseAmount) * match.priceUSD;
  }
...
function findOverridePrice(
  ...
  overrides: PriceOverrideMap | undefined,
  ...
  for (const item of INGREDIENT_CATALOG) {
    ...
    const entry = overrides[item.key];
    if (entry && keyWords.every((word) => n.includes(word))) {
      return { entry: item, priceUSD: entry.priceUSD };
    }
  }
  return null;
}

function costForIngredientUsd(ing: Ingredient, overrides?: PriceOverrides): number | null {
  const userMatch = findOverridePrice(ing.name, overrides?.userPrices);
  if (userMatch) return scaledOverrideCost(ing.name, ing.amount, userMatch);
  const aiMatch = findOverridePrice(ing.name, overrides?.aiPrices);
  if (aiMatch) return scaledOverrideCost(ing.name, ing.amount, aiMatch);
  return estimateIngredientCostUSD(ing.name, ing.amount);
}
...
export function estimateRecipeCost(
  ...
  for (const ing of ingredients) {
    const usd = costForIngredientUsd(ing, overrides);
    if (usd == null) continue;
    lines.push({ ingredient: ing, usd });
  }
  if (lines.length === 0) return null;

  const totalUSD = lines.reduce((sum, line) => sum + line.usd, 0);
  const serveCount = servings > 0 ? servings : 1;
  const pct = lines.length / ingredients.length;
  const confidence: RecipeCost['confidence'] = pct >= 0.75 ? 'high' : pct >= 0.5 ? 'medium' : 'low';

  return {
    totalUSD,
    perServeUSD: totalUSD / serveCount,
    coveredIngredients: lines.length,
...
export function parsePriceOverrideMap(raw: unknown): PriceOverrideMap {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {};
  const out: PriceOverrideMap = {};
  for (const [key, value] of Object.entries(raw)) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) continue;
    const rec = value as Record<string, unknown>;
    const price = Number(rec.priceUSD);
    const baseAmount = Number(rec.baseAmount);
    const baseUnit = typeof rec.baseUnit === 'string' ? rec.baseUnit : '';
    if (!Number.isFinite(price) || price <= 0 || !Number.isFinite(baseAmount) || baseAmount <= 0 || !baseUnit) continue;
    out[key] = { priceUSD: price, baseAmount, baseUnit };
  }
  return out;
}

export function catalogPricesFromAi(parsed: Record<string, unknown>): PriceOverrideMap {
  const prices: PriceOverrideMap = {};
  for (const item of INGREDIENT_CATALOG) {
    const raw = parsed[item.key];
    const rec = raw && typeof raw === 'object' && !Array.isArray(raw) ? (raw as Record<string, unknown>) : null;
    const aiPrice = rec ? Number(rec.priceUSD) : NaN;
    const priceUSD = Number.isFinite(aiPrice) && aiPrice > 0 ? aiPrice : item.defaultPriceUSD;
    prices[item.key] = {
      priceUSD: Math.max(0.001, priceUSD),
...
export function attachRecipeCost(
  recipe: Recipe,
  market: GroceryMarket,
  book?: IngredientPriceBook | null,
): Recipe {
  const cost = estimateRecipeCostForMarket(recipe, market, book);
  return cost ? { ...recipe, cost } : recipe;
}

export function estimateRecipeCostForMarket(
  recipe: Pick<Recipe, 'ingredients' | 'servings'>,
  market: GroceryMarket,
  book?: IngredientPriceBook | null,
): RecipeCostEstimate | null {
  const servings = recipe.servings && recipe.servings > 0 ? recipe.servings : 4;
  const raw = estimateRecipeCost(recipe, servings, { aiPrices: book?.prices ?? {} });
  if (!raw) return null;
  const multiplier = getCurrencyConfig(market.country).multiplier;
  const breakdown: RecipeCostLine[] = raw.lines.map((line) => ({
    name: line.ingredient.name,
    amount: line.ingredient.amount,
    lineCost: roundMoney(line.usd * multiplier),
  }));
  return {
    totalCost: roundMoney(raw.totalUSD * multiplier),
    costPerServe: roundMoney(raw.perServeUSD * multiplier),
    currency: market.currency,
    country: market.country,
    stores: [...market.stores],
    pricedAt: book?.pricedAt ?? '',
    servingsBasis: servings,
    breakdown,
    coveredIngredients: raw.coveredIngredients,
    totalIngredients: raw.totalIngredients,
  };
}
